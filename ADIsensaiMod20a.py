...
#ideas for both programs                                            4/04/2024 JRJ
...
#import dataDriver2a as dDx      #optional
VSlog,MRdat,Cre = hBx.callinfo.logging,hBx.callinfo.datetime,hBx.callinfo.re    #shortcuts for testing, last reg expr
vs,selL,selL2,lYr = '20a',0,3,True                      #2024 is 1! q=49 is significant
line,lastline,lend,nwLn,sep = '','','\r\n','\n',','     #2nd # above: best 0,5  good short 3,1  echo 2
...
ctFlgNm = [  'FwD',    'PwMC',   'FSp',    'PwD',    'PnC',    'FAT',    'StdC',  'Std',   'nVb',   'AlT',   'Rev',   'diag' ] #linked to arrays below
pgmCtFlgs = ['ttfttfx','FTTFTFx','100010x','ftfttfx','FFTFTFx','111111x','111 1 ','111011','11xx1x','111111','010111','t000t0' ] #6 in @
#above 2 used for commands, below for standard case; make a dictionary?
...
myDatL = ['','my_cnty','my_lat','my_lon','my_dxcc','my_gridsquare','my_cq_zone','my_itu_zone','my_rig','my_state',] # +enumerated 2 upStr
...
def opn_outp(k,k1):             # originally, samOutput for sample output                       5.0 p1
    'opens the output file for conversion to ADI, 1st program'
...
def sercLst(ch,ls):         #a primative for a few subr's
    if ch in ls:    idx = ls.index(ch)
    else:           idx = -len(ls)
    return idx              #returns neg if not found
...
#                print (426,'  cleared buffer, was:',b)
                return ''
            else:  print(421,'  read buffer, is:',b)
        else:                       #sp exists
            if lef:
...
def std_case(k,lin,g1):    #need a standard format so matching happens! use k?
    ...
    def adj_vals(v3,j0):                #for j1 loop
        j2 = j0.split(':')              #remove innards of < :l(:t)>
        fd,os1,os2 = True,'',''         #below are exclusive; add '>'?
        if j2[0] in upStr:
            os1 = '>'+v3.upper()
            lin = lin.replace('>'+v3,os1,1)
        elif j2[0] in capSt:
            os2 = '>'+v3.title()
            lin = lin.replace('>'+v3,os2,1)
        else:                           #above, alter value
            fd = False
#            print ( 29.4,'value not changed 4: ',tg[k3],'standardize',j2[0] )
        print ( 29.4,j0,j2,'os1 & 2:',os1,'|',os2,'|' )
        return fd
    
    lin = lin.lower()                  #get rid of the pesky comma w/ tilde or accent?
    #wrt_1linA(lin,497,g1)              #first view of
...
#    print (517,bg,ed,cn)
    ...
    wrt_1linA(lin,29.2,g1)                #2nd view of
    ...
    print ( 29.3,'extracted t&v: ',tp )      #strip inside tg? +1 would be at end of string...
    if len(tg)==0:  return str(l2)          #nothing found
    
    for j1 in tg:                           #loop thru found tags, should be 1 or more
        bk = adj_vals(vl[k3],j1)            #new
...
#        fd,lin = bk[0],bk[1]    #new
        rs += [bk]                          #store each result, then?
        k3 += 1                             #next
    wrt_1linA(lin,29.5,g1)                  #3rd/last view of
    print (29.6,rs,'values')                #boolean list
...
def get_Bol (s):                #creates boolean list
    ...
    while ct < 0:
        c = s[ct]
        vl = ( c=='T' or c=='t' or c=='1' or c=='y' or c=='Y' )  #                              31.0 used below
        lo += [vl]
        ct += 1
...
def rtnPflgs(pn):       #gets a control set, either pgm 1/2
    'uses boolean subroutine (prev.) to pick a program control flag set'
    ...
    print ( 32,'code:',ctFlgNm[idx],idx )
    return get_Bol(pgmCtFlgs[idx])

def ovrrFlgs(pgm,cmNo):     #use selLx to get the set (6/12 in array)
    'for both conversion types, allows override of the default command set during either pgm run'
    print ( 33, '  4 program:', pgm, 'the default Command Set is:', cmNo )
    pgm += -1 #convert to boolean; first 6 goes with pgm 1
    if pgm:  print ( 'the choices are:', ctFlgNm[cmNp1:], 'set2' )
    else:    print ( 'the choices are:', ctFlgNm[0:cmNp1], 'set1' )
    uin = input('do you want to override the current flags? y/n ')
    if uin != 'y':  return (-1,'')      #do nothing                                             33.0 in both 1 & 2
    ui2 = input('enter a Mnemonic command string? ~3 ch. ')   #new Mnemonic another input
    si = sercLst(ui2,ctFlgNm)
    ...
    return (si,ui2)         #new string/ default; tf equiv.
...
def getHr(s):
    ...
    Wzns = ['Z','A','B','C','D','E','F','G','H','I','K','L','M']    #hours behind UTC, EST zone R; 4 UTC conversion 1st program
...
def Paddd(s):                           #shows how much too!
    'the Npadd function inverted, but now tells how much padding too!'
    l1,l2 = len(s.strip()),len(s)
    return (l2-l1)                                                                              # (42.0)

# add meta class statement? make sure methods have access to the globals herein?
class Hart:
    'test class: variable names, wrapper for functions from above; write example program to try?'
...
# subroutine testing:
print ( '----------begin conversion subroutine(s) testing:',tmRel(),'epoch time----------' )
if __name__ == '__main__':
    print ( solv_tof(13,3,8),solv_tof(22,44,16),solv_tof(23,55,45),solv_tof(17,45,24),solv_tof(2,4,2),solv_tof(7,1,5) ) #but add more
    ...
    print ( nwLn, __name__, nwLn, dir(__name__), nwLn, __doc__, nwLn, 'end of?', nwLn ) # the current set?
    print ( dir (hBx), nwLn, '  end of:', hBx.__name__, nwLn ) # the imported pyHamtools
